//! Chapter 8, Lesson 2: Optimal Proportional Navigation Adjoint
//!
//! Adjoint analysis for optimal proportional navigation with first-order lag.
...
pub fn run() -> Results {
    ...
    let vm: f64 = 3000.0;
    let hedeg: f64 = -20.0;
    let apn: i32 = 0;  // 0 = PN, 1 = APN, 2 = OPN
    let h: f64 = 0.01;
    let he = hedeg / 57.3;
    ...
    while tp <= (tf - 1e-5) {
        ...
        let (c1, c2, c3, c4) = match apn {
            0 => {
                // Standard PN
                (xnp / (tgo * tgo), xnp / tgo, 0.0, 0.0)
            }
            1 => {
                // Augmented PN
                (xnp / (tgo * tgo), xnp / tgo, 0.5 * xnp, 0.0)
            }
            _ => {
                // Optimal PN
                let x = tgo / tau;
...
pub fn run_and_save(output_dir: &str) -> std::io::Result<Results> {
    ...
    let data_file = format!("{}/c8l2_datfil.txt", output_dir);
    save_data(&data_file, &[
        results.time.clone(),
        results.xmnt.clone(),
        results.xmhe.clone(),
    ...
    let plot_file = format!("{}/c8l2_plot.png", output_dir);
    let config = PlotConfig::new("Target Maneuver Miss (PN)")
    ...
    println!("C8L2: Simulation finished");
