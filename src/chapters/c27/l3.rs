//! Chapter 27, Lesson 3: Miss for Various TGO Maneuver Starting Times
//!
//! Kalman filter-based estimation with various APN options.
//! Computes miss distance for different target maneuver starting times.
...
pub fn run() -> Results {
    ...
    while tstart <= 10.0 {
        ...
        let mut p12: f64 = 0.0;
        let mut p13: f64 = 0.0;
        let mut p22 = (vm * hedegf / 57.3).powi(2);
        let mut p23: f64 = 0.0;
        let mut p33 = xnt * xnt;
        ...
        while t <= tf - 1e-5 {
            ...
            if s >= ts - 1e-5 {
                ...
                if apn == 0 {
                    ...
                    let c1 = xnpp / tgo.powi(2);
                    let c2 = xnpp / tgo;
                    let c3 = 0.5 * xnpp;
                    let c4 = -xnpp * ((-x).exp() + x - 1.0) / (x * x);
...
pub fn run_and_save(output_dir: &str) -> std::io::Result<Results> {
    ...
    let data_file = format!("{}/c27l3_datfil.txt", output_dir);
    save_data(&data_file, &[
        results.tgos.clone(),
        results.y.clone(),
    ])?;

    let plot_file = format!("{}/c27l3_miss.png", output_dir);
    let config = PlotConfig::new("Miss for Various TGO Maneuver Starting Times")
    ...
    println!("C27L3: Simulation finished");
