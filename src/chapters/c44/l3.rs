//! Chapter 44, Lesson 3: BMD Interceptor Footprint Analysis
//!
//! Ballistic missile defense: interceptor footprint (defender location coverage).
...
pub fn run() -> Results {
    ...
    let mut array_xlongm = Vec::new();
    let mut array_xlatm = Vec::new();
    let mut array_vbom = Vec::new();

    let mut xlongmdeg: f64 = 0.0;
    while xlongmdeg <= 150.0 {
        let mut xlatmdeg: f64 = -40.0;
        while xlatmdeg <= 40.0 {
            ...
            while tlaunch <= 1600.0 {
                ...
                while tf <= tftot {
                    ...
                    // Calculate target states at desired intercept time using Kepler
                    ...
                    // Calculate missile velocity required to intercept
                    ...
                    if vbom < vbolim && vbot < 7.5 {
                        ...
                        array_xlongm.push(xlongmdeg * 111.0);
                        array_xlatm.push(xlatmdeg * 111.0);
...
pub fn run_and_save(output_dir: &str) -> std::io::Result<Results> {
    ...
    let data_file = format!("{}/c44l3_datfil.txt", output_dir);
    save_data(&data_file, &[
        results.tf.clone(),
        results.tlaunch.clone(),
        results.xlongm.clone(),
        results.xlatm.clone(),
        results.vbom.clone(),
    ])?;

    println!("C44L3: Simulation finished");
...
mod tests {
    ...
    fn test_c44l3_runs() {
        let results = run();
        // May have results depending on geometry
        assert!(true);
    }
}
