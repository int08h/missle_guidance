//! Chapter 26, Lesson 7: Frequency Response Analysis (Bode Plot)
//!
//! Computes gain and phase frequency response for open-loop transfer function.
...
pub fn run() -> Results {
    ...
    for i in 2..=160 {
        ...
        let top = 13500.0 * (1.0 + (w / 29.1).powi(2)).sqrt();
        let bot = w * (1.0 + (w / 2.0).powi(2)).sqrt();
        let xmag = top / bot;
...
pub fn run_and_save(output_dir: &str) -> std::io::Result<Results> {
    ...
    let data_file = format!("{}/c26l7_datfil.txt", output_dir);
    save_data(&data_file, &[
        results.freq.clone(),
        results.gain.clone(),
        results.phase.clone(),
    ...
    let plot_file = format!("{}/c26l7_gain.png", output_dir);
    ...
    println!("C26L7: Simulation finished");
