//! Chapter 22, Lesson 4: Rate Gyro System with Actuator Dynamics and Delay
//!
//! Implements rate gyro flight control system with second-order actuator
//! dynamics and time delay using a circular buffer.
...
pub fn run() -> Results {
    ...
    // Delay buffer
    let mut z: Vec<f64> = vec![0.0; 20002];
    let mut i: usize = 1;
    let dint: usize = (delay / h).floor() as usize;
    ...
    while t <= 0.99999 {
        ...
        // Store delayed command (use delcp2 from after Euler step for consistency with MATLAB)
        z[i] = delcp2;
...
pub fn run_and_save(output_dir: &str) -> std::io::Result<Results> {
    ...
    let data_file = format!("{}/c22l4_datfil.txt", output_dir);
    save_data(&data_file, &[
        results.time.clone(),
        results.xnl.clone(),
        results.xncg.clone(),
    ...
    let plot_file = format!("{}/c22l4_accel.png", output_dir);
    let config = PlotConfig::new("Rate Gyro with Actuator and Delay")
    ...
    println!("C22L4: Simulation finished");
