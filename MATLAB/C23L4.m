...
while T<=(TF-.00001)
	...
	while STEP <=1
        ...
        CN=2*ALF+1.5*SPLAN*ALF*ALF/SREF+8*SWING*ALF/(B*SREF)...
            +8*STAIL*(ALF+DELR)/(B*SREF);
        CM=2*ALF*TMP4+1.5*SPLAN*ALF*ALF*TMP3/SREF...
            +8*SWING*ALF*TMP1/(B*SREF)+8*STAIL*(ALF+DELR)...
            *TMP2/(B*SREF);
        ...
        XD=WI*(THDDEG+XKA*(XNLG-XNCG*XKDC));
 		FLAG=1;
	end
	FLAG=0;
...
figure
plot(ArrayT,ArrayXNLG,ArrayT,ArrayXNCG),grid
xlabel('Time (Sec)')
ylabel('Acceleration (G) ')
clc
output=[ArrayT',ArrayXNLG',ArrayXNCG'];
