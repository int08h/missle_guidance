...
while RM@ >= 0
    ...
    while STEP <=1
        ...
        elseif IGUID==2
            EPS=GAM-XLAM;
            TEMP1=(XN-2+2*cos(EPS))*VM*(TFDES-T)-XN*RTM;
            X2=.5*(XN-1)*TEMP1/(TFDES-T)^2;
            GAMD=2*X2/(VM*sin(EPS))+XLAMD;
            XNC=VM*GAMD;
        end
        if XNC>XNCLIM
            XNC=XNCLIM;
    ...
    FLAG=0;
    GAM=.5*(GAMOLD+GAM+H*GAMD);
    XLAM=.5*(XLAMOLD+XLAM+H*XLAMD);
...
output=[ArrayT',ArrayRT1K',ArrayRT2K',ArrayRM1K',ArrayRM2K',...
     ArrayXNCG',ArrayEPSDEG',ArrayRTM'];
